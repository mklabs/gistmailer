...
function Git(options) {
  ...
  this.basedir = this.options.basedir || path.join(__dirname, '../tmp/templates');
...
Git.prototype.clone = function clone(url, done) {
  ...
  url = !/\.git$/.test(url) ? 'https://gist.github.com/' + url + '.git' : url;

  var dir = this.directory(this.name || url);
  var me = this;
